...
public class RESTAccess {
	 ...
	 * 
	 */
	public MetricDatas getRESTMetricQueryCustom(
			String  application, 
			String  metricPath,
			long    start, 
			long    end, 
			boolean rollup)
	{
		...
		if(s.debugLevel >= 2)
		{
			String s = String.format(
					"\nQuery Index sent: app=%s, metricPath=%s, start=%d, end=%d, rollup=%d",
					application, metricPath, start, end, rollup);
		...
		query = mq.queryCustomMetrics(application, metricPath, start, end, rollup);
		...
		try{
			...
			logger.log(Level.SEVERE,
					new StringBuilder().append("Exception occurred executing REST query::\n").append(e.getMessage()).append("\n").toString());
		}
		return null;
